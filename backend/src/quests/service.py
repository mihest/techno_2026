...
class QuestService:
    @classmethod
    async def _to_detail_response(cls, session: AsyncSession, quest) -> QuestDetailResponse:
    ...
    @classmethod
    async def get_quests_for_moderation(cls, session: AsyncSession, offset: int, limit: int) -> QuestListResponse:
        items, total = await QuestDAO.list_for_moderation(session, offset=offset, limit=limit)
    ...
    async def moderate_quest(
        ...
        if quest.status != QuestStatusEnum.MODERATION:
            raise HTTPException(status_code=400, detail="Quest is not on moderation")

        if decision.action == "publish":
            quest = await QuestDAO.set_moderation_publish(session, quest)
            return await cls._to_detail_response(session, quest)

        reason = (decision.reason or "").strip()
        if not reason:
            raise HTTPException(status_code=422, detail="Reject reason is required")
        quest = await QuestDAO.set_moderation_reject(session, quest, reason)
        return await cls._to_detail_response(session, quest)
